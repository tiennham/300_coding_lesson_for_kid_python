...
class Solution:
    def search(self, nums: List[int], target: int) -> int:
        ...
        return self.while_search(nums, target)

    # hàm này là tôi cứng đầu, cố chấp:
    # có ý nghĩa chứng minh: những ý tưởng hợp lý phải triển khai được
    # ý tưởng là sẽ tracking lại quá trình tăng giảm index, theo đó cộng / trừ để ra được index cần tìm
    def while_search2(self, nums: List[int], target: int):
        track_idx = 0
        if len(nums) % 2 == 0:
            track_idx += (int(len(nums) / 2))
        else:
            track_idx += (int(len(nums) / 2))
        ...
        while count <= (int(len(nums) / 2)):
            ...
            middle_idx = int(end / 2)
            middle_val = sub_nums[middle_idx]

            if sub_nums[middle_idx] == target:
                return track_idx

            if middle_val > target:
                sub_nums = sub_nums[start:middle_idx]
                if len(sub_nums) % 2 == 0:
                    track_idx -= (int(len(sub_nums) / 2))
                else:
                    track_idx -= (int(len(sub_nums) / 2) + 1)
            else:
                sub_nums = sub_nums[middle_idx:end]
                track_idx += (int(len(sub_nums) / 2))

        return -1

    def while_search(self, nums: List[int], target: int):
        start = 0
        end = len(nums)
        while start < end:
    ...
    def binary_search(self, start, end, nums: List[int], target: int):
...
# nums = [-1, 0, 3, 5, 9, 12, 14, 17, 19, 21, 23, 25, 39, 49, 59, 69, 79, 89, 99, 111]
# target = 9
# nums = [2,5]
# target = 5
